...
out_path="${run_dir}/artifacts/phase13_verify/verification.json"
cmd=(uv run python -m src.utils.phase13_verify --run-dir "${run_dir}" --confirm-batches 16 --out "${out_path}")
if [[ "$#" -eq 0 ]]; then
  set -- \
