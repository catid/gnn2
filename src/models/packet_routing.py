...
class PacketRoutingModel(nn.Module):
    def __init__(self, config: dict[str, Any]):
        ...
        self.factorized_content_slot_count = int(config.get("factorized_content_slot_count", 1))
        if self.factorized_content_slot_count <= 0:
            raise ValueError("factorized_content_slot_count must be positive.")
        self.factorized_content_slot_mode = str(config.get("factorized_content_slot_mode", "none"))
        if self.factorized_content_slot_mode == "none" and self.factorized_content_slot_count != 1:
            raise ValueError(
                "factorized_content_slot_mode='none' requires factorized_content_slot_count == 1."
            )
        self.factorized_content_slot_query_mode = str(
            config.get("factorized_content_slot_query_mode", "attention")
        )
        self.factorized_content_slot_scale = float(config.get("factorized_content_slot_scale", 1.0))
        self.factorized_content_slot_zero_init = bool(
            config.get("factorized_content_slot_zero_init", False)
        )
        ...
        self.factorized_content_sidecar_write_heads = int(
            config.get("factorized_content_sidecar_write_heads", 2)
        )
        ...
        self.factorized_content_slot_proj = None
        self.factorized_content_slot_embeddings = None
        self.factorized_content_slot_query_proj = None
        ...
        self.factorized_content_sidecar_key_proj = None
        self.factorized_content_sidecar_value_proj = None
        self.factorized_content_sidecar_query_proj = None
        ...
        if self.readout_mode == "plain":
            ...
            if self.factorized_content_slot_mode == "independent":
                if self.factorized_content_slot_count <= 1:
                    raise ValueError("factorized_content_slot_mode='independent' requires at least 2 slots.")
                self.factorized_content_slot_proj = nn.Linear(
                    self.hidden_dim,
                    self.hidden_dim * self.factorized_content_slot_count,
                )
                if self.factorized_content_slot_zero_init:
                    nn.init.zeros_(self.factorized_content_slot_proj.weight)
                    nn.init.zeros_(self.factorized_content_slot_proj.bias)
            elif self.factorized_content_slot_mode == "shared":
                if self.factorized_content_slot_count <= 1:
                    raise ValueError("factorized_content_slot_mode='shared' requires at least 2 slots.")
                self.factorized_content_slot_proj = nn.Linear(self.hidden_dim, self.hidden_dim)
                self.factorized_content_slot_embeddings = nn.Parameter(
                    torch.randn(self.factorized_content_slot_count, self.hidden_dim) * 0.02
                )
                if self.factorized_content_slot_zero_init:
                    nn.init.zeros_(self.factorized_content_slot_proj.weight)
                    nn.init.zeros_(self.factorized_content_slot_proj.bias)
                    nn.init.zeros_(self.factorized_content_slot_embeddings)
            elif self.factorized_content_slot_mode != "none":
                raise ValueError(f"Unknown factorized_content_slot_mode: {self.factorized_content_slot_mode}")
            if self.factorized_content_slot_mode != "none":
                if self.factorized_content_slot_query_mode == "attention":
                    self.factorized_content_slot_query_proj = nn.Linear(
                        self.hidden_dim,
                        self.hidden_dim,
                        bias=False,
                    )
                elif self.factorized_content_slot_query_mode != "mean":
                    raise ValueError(
                        "factorized_content_slot_query_mode must be one of: attention, mean"
                    )
            if self.factorized_content_sidecar_mode == "residual_mlp":
                ...
                if self.factorized_content_sidecar_mode == "trajectory_content_multihead_reserved_mixed_write_value_gated_kv_memory":
                    self.factorized_content_sidecar_write_mix_proj = nn.Linear(
                        self.hidden_dim,
                        self.factorized_content_sidecar_write_heads,
                        bias=False,
                    )
                if self.factorized_content_sidecar_mode == "trajectory_content_multihead_reserved_temperature_write_value_gated_kv_memory":
                    self.factorized_content_sidecar_write_reserved_temperature_proj = nn.Linear(
                        self.hidden_dim,
                        self.factorized_content_sidecar_write_heads,
                        bias=False,
                    )
                if self.factorized_content_sidecar_mode == "trajectory_content_multihead_reserved_shared_penalty_write_value_gated_kv_memory":
                    self.factorized_content_sidecar_write_shared_penalty_proj = nn.Linear(
                        self.hidden_dim,
                        self.factorized_content_sidecar_write_heads,
                        bias=False,
                    )
                if self.factorized_content_sidecar_mode == "trajectory_content_multihead_reserved_strength_penalty_write_value_gated_kv_memory":
                    self.factorized_content_sidecar_write_reserved_strength_penalty_proj = nn.Linear(
                        self.hidden_dim,
                        self.factorized_content_sidecar_write_heads,
                        bias=False,
                    )
                ...
                if (
                    ...
                    self.factorized_content_sidecar_write_source_proj = nn.Linear(
                        self.hidden_dim,
                        self.hidden_dim * self.factorized_content_sidecar_write_heads,
                        bias=False,
                    )
    ...
    def _factorized_readout_input(
        ...
        *,
        ...
        base_content = self.factorized_content_proj(raw_content)
        ...
        if self.factorized_content_slot_mode != "none":
            if self.factorized_content_slot_mode == "independent":
                slot_hidden = self.factorized_content_slot_proj(raw_content).view(
                    raw_content.shape[0],
                    self.factorized_content_slot_count,
                    self.hidden_dim,
                )
            else:
                shared_slot = self.factorized_content_slot_proj(raw_content).unsqueeze(1)
                slot_embeddings = self.factorized_content_slot_embeddings.to(
                    device=raw_content.device,
                    dtype=raw_content.dtype,
                )
                slot_hidden = shared_slot + slot_embeddings.unsqueeze(0)
            if self.factorized_content_slot_query_mode == "mean":
                slot_weights = torch.full(
                    (raw_content.shape[0], self.factorized_content_slot_count),
                    1.0 / self.factorized_content_slot_count,
                    device=raw_content.device,
                    dtype=raw_content.dtype,
                )
            else:
                slot_query = self.factorized_content_slot_query_proj(query_hidden)
                slot_scores = (slot_hidden * slot_query.unsqueeze(1)).sum(dim=-1) / math.sqrt(self.hidden_dim)
                slot_weights = torch.softmax(slot_scores, dim=-1)
            slot_context = (slot_weights.unsqueeze(-1) * slot_hidden).sum(dim=1)
            content = content + (self.factorized_content_slot_scale * slot_context)
        sidecar_hidden = None
        sidecar_slots = None
        sidecar_weights = None
        sidecar_write_weights = None
        sidecar_value_gate = None
        sidecar_reserved_temperature = None
        sidecar_shared_penalty = None
        sidecar_reserved_strength_penalty = None
        sidecar_reserved_margin = None
        if self.factorized_content_sidecar_mode != "none":
            ...
            if self.factorized_content_sidecar_mode in {
                ...
                if self.factorized_content_sidecar_mode in {
                    ...
                    write_query = write_query + self.factorized_content_sidecar_write_content_proj(content).view(
                        content.shape[0],
                        self.factorized_content_sidecar_write_heads,
                        self.hidden_dim,
                    )
                    if sidecar_route_features is not None and self.trajectory_bank_route_proj is not None:
                        write_query = write_query + self.trajectory_bank_route_proj(sidecar_route_features).unsqueeze(1)
                    if self.factorized_content_sidecar_mode == "trajectory_content_multihead_headwise_write_value_gated_kv_memory":
                        write_source_slots = self.factorized_content_sidecar_write_source_proj(sidecar_slots).view(
                            sidecar_slots.shape[0],
                            sidecar_slots.shape[1],
                            self.factorized_content_sidecar_write_heads,
                            self.hidden_dim,
                        )
                        write_scores = torch.einsum("bshd,bhd->bhs", write_source_slots, write_query) / math.sqrt(
                            self.hidden_dim
                        )
                    else:
                        write_scores = torch.einsum("bsd,bhd->bhs", sidecar_slots, write_query) / math.sqrt(
                            self.hidden_dim
                        )
                    topk = min(max(1, self.factorized_content_sidecar_write_topk), sidecar_slots.shape[1])
                    if self.factorized_content_sidecar_mode == "trajectory_content_multihead_disjoint_write_value_gated_kv_memory":
                        assignment_mask = torch.zeros_like(write_scores, dtype=torch.bool)
                        assignment_values = torch.zeros_like(write_scores)
                        ...
                        for _ in range(topk):
                            for head_idx in range(write_scores.shape[1]):
                                ...
                                chosen_scores = write_scores[:, head_idx, :].gather(1, chosen_idx)
                                valid_choice = has_available.unsqueeze(-1).to(dtype=write_scores.dtype)
                                head_hot = torch.zeros_like(write_scores[:, head_idx, :])
                                head_hot.scatter_(1, chosen_idx, valid_choice)
                                assignment_mask[:, head_idx, :] |= head_hot > 0.0
                                assignment_values[:, head_idx, :] = (
                                    assignment_values[:, head_idx, :] + (head_hot * write_scores[:, head_idx, :])
                                )
                        ...
                        if bool(unassigned.any().item()):
                            ...
                            fallback_scores = write_scores.gather(-1, fallback_idx).squeeze(-1)
                            for head_idx in range(write_scores.shape[1]):
                                ...
                                assignment_mask[:, head_idx, :] |= fallback_hot > 0.0
                                assignment_values[:, head_idx, :] = (
                                    assignment_values[:, head_idx, :]
                                    + (fallback_hot * write_scores[:, head_idx, :])
                                )
                        ...
                        reserved_scores = torch.where(
                            reserved_mask,
                            reserved_values,
                            torch.full_like(write_scores, float("-inf")),
                        )
                        reserved_weights = torch.softmax(reserved_scores, dim=-1)
                        shared_k = min(max(0, topk - 1), sidecar_slots.shape[1] - 1)
                        if shared_k > 0:
                            fallback_scores = write_scores.masked_fill(reserved_mask, float("-inf"))
                            shared_values, shared_idx = torch.topk(fallback_scores, k=shared_k, dim=-1)
                            shared_masked_scores = torch.full_like(write_scores, float("-inf"))
                            shared_masked_scores.scatter_(2, shared_idx, shared_values)
                            shared_weights = torch.softmax(shared_masked_scores, dim=-1)
                            mix_gate = torch.sigmoid(self.factorized_content_sidecar_write_mix_proj(content)).unsqueeze(-1)
                            sidecar_write_weights = (mix_gate * reserved_weights) + ((1.0 - mix_gate) * shared_weights)
                        else:
                            sidecar_write_weights = reserved_weights
                    elif (
                        self.factorized_content_sidecar_mode
                        == "trajectory_content_multihead_reserved_temperature_write_value_gated_kv_memory"
                    ):
                        ...
                        sidecar_reserved_temperature = 0.5 + (
                            1.5
                            * torch.sigmoid(
                                self.factorized_content_sidecar_write_reserved_temperature_proj(content)
                            )
                        )
                        masked_write_scores = torch.where(
                            reserved_mask,
                            reserved_values / sidecar_reserved_temperature.unsqueeze(-1),
                            torch.full_like(write_scores, float("-inf")),
                        )
                        shared_k = min(max(0, topk - 1), sidecar_slots.shape[1] - 1)
                        if shared_k > 0:
                        ...
                        shared_k = min(max(0, topk - 1), sidecar_slots.shape[1] - 1)
                        if shared_k > 0:
                            reserved_by_any = reserved_mask.any(dim=1, keepdim=True)
                            sidecar_shared_penalty = 2.0 * torch.sigmoid(
                                self.factorized_content_sidecar_write_shared_penalty_proj(content)
                            )
                            penalty_mask = reserved_by_any.expand(-1, write_scores.shape[1], -1)
                            penalty_values = penalty_mask.to(dtype=write_scores.dtype) * sidecar_shared_penalty.unsqueeze(-1)
                            fallback_scores = write_scores.masked_fill(reserved_mask, float("-inf")) - penalty_values
                            shared_values, shared_idx = torch.topk(fallback_scores, k=shared_k, dim=-1)
                            masked_write_scores.scatter_(2, shared_idx, shared_values)
                    elif (
                        self.factorized_content_sidecar_mode
                        ...
                        shared_k = min(max(0, topk - 1), sidecar_slots.shape[1] - 1)
                        if shared_k > 0:
                            reservation_strength = reserved_values.max(dim=1).values.unsqueeze(1)
                            sidecar_reserved_strength_penalty = 2.0 * torch.sigmoid(
                                self.factorized_content_sidecar_write_reserved_strength_penalty_proj(content)
                            )
                            fallback_scores = write_scores.masked_fill(reserved_mask, float("-inf")) - (
                                reservation_strength * sidecar_reserved_strength_penalty.unsqueeze(-1)
                            )
                            shared_values, shared_idx = torch.topk(fallback_scores, k=shared_k, dim=-1)
                            masked_write_scores.scatter_(2, shared_idx, shared_values)
                    elif (
                        ...
                        if shared_k > 0:
                            fallback_scores = write_scores.masked_fill(reserved_mask, float("-inf"))
                            fallback_available = torch.isfinite(fallback_scores)
                            fallback_best = fallback_scores.max(dim=-1).values
                            reserved_score = reserved_values.max(dim=-1).values
                            sidecar_reserved_margin = torch.where(
                                fallback_available.any(dim=-1),
                                reserved_score - fallback_best,
                                torch.zeros_like(reserved_score),
                            )
                            alloc_priority = torch.where(
                                fallback_available.any(dim=-1),
                                sidecar_reserved_margin,
                                torch.full_like(sidecar_reserved_margin, float("-inf")),
                            )
                            shared_budget = torch.zeros_like(sidecar_reserved_margin, dtype=torch.long)
                            total_shared_budget = shared_k * write_scores.shape[1]
                            for _ in range(total_shared_budget):
                                remaining_capacity = fallback_available.sum(dim=-1) - shared_budget
                                alloc_scores = alloc_priority - shared_budget.to(dtype=write_scores.dtype)
                                alloc_scores = torch.where(
                                    remaining_capacity > 0,
                                    alloc_scores,
                                    torch.full_like(alloc_scores, float("-inf")),
                                )
                                has_budget_target = torch.isfinite(alloc_scores).any(dim=-1)
                                if not bool(has_budget_target.any().item()):
                                    break
                                chosen_head = alloc_scores.argmax(dim=-1, keepdim=True)
                                shared_budget.scatter_add_(
                                    1,
                                    chosen_head,
                                    has_budget_target.unsqueeze(-1).to(dtype=shared_budget.dtype),
                                )
                            for head_idx in range(write_scores.shape[1]):
                                head_budget = shared_budget[:, head_idx]
                                max_head_budget = int(head_budget.max().item())
                                if max_head_budget <= 0:
                                    continue
                                head_fallback_scores = fallback_scores[:, head_idx, :]
                                head_values, head_idxs = torch.topk(
                                    head_fallback_scores,
                                    k=max_head_budget,
                                    dim=-1,
                                )
                                budget_mask = (
                                    torch.arange(max_head_budget, device=write_scores.device).unsqueeze(0)
                                    < head_budget.unsqueeze(-1)
                                )
                                head_values = torch.where(
                                    budget_mask,
                                    head_values,
                                    torch.full_like(head_values, float("-inf")),
                                )
                                masked_write_scores[:, head_idx, :].scatter_(1, head_idxs, head_values)
                        ...
                        masked_write_scores.scatter_(2, topk_idx, topk_values)
                    else:
                        masked_write_scores = write_scores
                    if sidecar_write_weights is None:
        ...
        if self.factorized_combiner_mode == "concat":
            ...
            readout_input = self.factorized_combiner(torch.cat([gated, query_hidden], dim=-1))
        elif self.factorized_combiner_mode == "bilinear":
            readout_input = torch.tanh(self.factorized_bilinear(content, query_hidden))
        elif self.factorized_combiner_mode == "film":
            gamma, beta = torch.chunk(self.factorized_film(query_hidden), 2, dim=-1)
            readout_input = content * (1.0 + torch.tanh(gamma)) + beta
        else:
            raise ValueError(f"Unknown factorized_combiner_mode: {self.factorized_combiner_mode}")
        return readout_input, {
            "factorized_content_base_hidden": base_content,
            "factorized_content_hidden": content,
            "factorized_query_hidden": query_hidden,
            "factorized_content_slot_hidden": slot_hidden,
            "factorized_content_slot_weights": slot_weights,
            "factorized_content_slot_context": slot_context,
            ...
            "factorized_content_sidecar_reserved_temperature": sidecar_reserved_temperature,
            "factorized_content_sidecar_shared_penalty": sidecar_shared_penalty,
            "factorized_content_sidecar_reserved_strength_penalty": sidecar_reserved_strength_penalty,
            "factorized_content_sidecar_reserved_margin": sidecar_reserved_margin,
        }
    ...
    def forward(
        ...
        factorized_content_base_hidden = None
        factorized_query_hidden = None
        factorized_content_slot_hidden = None
        factorized_content_slot_weights = None
        factorized_content_slot_context = None
        ...
        factorized_content_sidecar_reserved_temperature = None
        factorized_content_sidecar_shared_penalty = None
        factorized_content_sidecar_reserved_strength_penalty = None
        factorized_content_sidecar_reserved_margin = None
        if self.readout_mode == "probe_query_views":
            ...
            readout_input, factorized_trace = self._factorized_readout_input(
                sink_state=sink_state,
                observations=observations,
                trace=trace,
                first_exit_time=first_exit_time,
            ...
            factorized_content_base_hidden = factorized_trace.get("factorized_content_base_hidden")
            factorized_content_hidden = factorized_trace.get("factorized_content_hidden")
            factorized_query_hidden = factorized_trace.get("factorized_query_hidden")
            factorized_content_slot_hidden = factorized_trace.get("factorized_content_slot_hidden")
            factorized_content_slot_weights = factorized_trace.get("factorized_content_slot_weights")
            factorized_content_slot_context = factorized_trace.get("factorized_content_slot_context")
            ...
            factorized_content_sidecar_reserved_temperature = factorized_trace.get(
                "factorized_content_sidecar_reserved_temperature"
            )
            factorized_content_sidecar_shared_penalty = factorized_trace.get(
                "factorized_content_sidecar_shared_penalty"
            )
            factorized_content_sidecar_reserved_strength_penalty = factorized_trace.get(
                "factorized_content_sidecar_reserved_strength_penalty"
            )
            factorized_content_sidecar_reserved_margin = factorized_trace.get(
                "factorized_content_sidecar_reserved_margin"
            )
            readout_input = self._apply_readout_adapter(readout_input)
        else:
            baseline_view = self._baseline_readout_input(
        ...
        if trace is not None:
            ...
            if factorized_content_base_hidden is not None:
                trace["factorized_content_base_hidden"] = factorized_content_base_hidden
            ...
            if factorized_content_slot_hidden is not None:
                trace["factorized_content_slot_hidden"] = factorized_content_slot_hidden
            if factorized_content_slot_weights is not None:
                trace["factorized_content_slot_weights"] = factorized_content_slot_weights
            if factorized_content_slot_context is not None:
                trace["factorized_content_slot_context"] = factorized_content_slot_context
            ...
            if factorized_content_sidecar_slots is not None:
            ...
            if factorized_content_sidecar_reserved_temperature is not None:
                trace["factorized_content_sidecar_reserved_temperature"] = (
                    factorized_content_sidecar_reserved_temperature
                )
            if factorized_content_sidecar_shared_penalty is not None:
                trace["factorized_content_sidecar_shared_penalty"] = factorized_content_sidecar_shared_penalty
            if factorized_content_sidecar_reserved_strength_penalty is not None:
                trace["factorized_content_sidecar_reserved_strength_penalty"] = (
                    factorized_content_sidecar_reserved_strength_penalty
                )
            if factorized_content_sidecar_reserved_margin is not None:
                trace["factorized_content_sidecar_reserved_margin"] = (
                    factorized_content_sidecar_reserved_margin
                )
        ...
        factorized_content_slot_entropy = torch.zeros((), device=device, dtype=dtype)
        factorized_content_slot_top1_weight = torch.zeros((), device=device, dtype=dtype)
        if factorized_content_slot_weights is not None:
            slot_weights = factorized_content_slot_weights.clamp_min(1e-8)
            factorized_content_slot_entropy = (
                -(slot_weights * slot_weights.log()).sum(dim=-1).mean().to(device=device, dtype=dtype)
            )
            factorized_content_slot_top1_weight = (
                factorized_content_slot_weights.max(dim=-1).values.mean().to(device=device, dtype=dtype)
            )
        factorized_content_sidecar_entropy = torch.zeros((), device=device, dtype=dtype)
        factorized_content_sidecar_top1_weight = torch.zeros((), device=device, dtype=dtype)
        if factorized_content_sidecar_weights is not None:
            sidecar_weights = factorized_content_sidecar_weights.clamp_min(1e-8)
            factorized_content_sidecar_entropy = (
                -(sidecar_weights * sidecar_weights.log()).sum(dim=-1).mean().to(device=device, dtype=dtype)
            )
        ...
        factorized_content_sidecar_reserved_temperature_mean = torch.zeros((), device=device, dtype=dtype)
        if factorized_content_sidecar_reserved_temperature is not None:
            factorized_content_sidecar_reserved_temperature_mean = (
                factorized_content_sidecar_reserved_temperature.mean().to(device=device, dtype=dtype)
            )
        factorized_content_sidecar_shared_penalty_mean = torch.zeros((), device=device, dtype=dtype)
        if factorized_content_sidecar_shared_penalty is not None:
            factorized_content_sidecar_shared_penalty_mean = (
                factorized_content_sidecar_shared_penalty.mean().to(device=device, dtype=dtype)
            )
        factorized_content_sidecar_reserved_strength_penalty_mean = torch.zeros((), device=device, dtype=dtype)
        if factorized_content_sidecar_reserved_strength_penalty is not None:
            factorized_content_sidecar_reserved_strength_penalty_mean = (
                factorized_content_sidecar_reserved_strength_penalty.mean().to(device=device, dtype=dtype)
            )
        factorized_content_sidecar_reserved_margin_mean = torch.zeros((), device=device, dtype=dtype)
        if factorized_content_sidecar_reserved_margin is not None:
            factorized_content_sidecar_reserved_margin_mean = (
                factorized_content_sidecar_reserved_margin.mean().to(device=device, dtype=dtype)
            )
        ...
        stats = {
            ...
            "factorized_content_slot_entropy": torch.full_like(
                accuracy, float(factorized_content_slot_entropy.detach().item())
            ),
            "factorized_content_slot_top1_weight": torch.full_like(
                accuracy, float(factorized_content_slot_top1_weight.detach().item())
            ),
            "factorized_content_sidecar_entropy": torch.full_like(
                accuracy, float(factorized_content_sidecar_entropy.detach().item())
            ),
            ...
            "factorized_content_sidecar_reserved_temperature_mean": torch.full_like(
                accuracy, float(factorized_content_sidecar_reserved_temperature_mean.detach().item())
            ),
            "factorized_content_sidecar_shared_penalty_mean": torch.full_like(
                accuracy, float(factorized_content_sidecar_shared_penalty_mean.detach().item())
            ),
            "factorized_content_sidecar_reserved_strength_penalty_mean": torch.full_like(
                accuracy, float(factorized_content_sidecar_reserved_strength_penalty_mean.detach().item())
            ),
            "factorized_content_sidecar_reserved_margin_mean": torch.full_like(
                accuracy, float(factorized_content_sidecar_reserved_margin_mean.detach().item())
            ),
