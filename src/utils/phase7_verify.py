...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Recompute and cross-check phase-7 metrics from a saved checkpoint.")
...
def main() -> None:
    ...
    for idx, eval_config_path in enumerate(args.eval_config):
        ...
        verification_payload["evaluations"][f"eval_{idx}"] = {
            "config_path": eval_config_path,
            "metrics": metrics,
            "comparisons": comparisons,
        }

    out_path = Path(args.out) if args.out else run_dir / "artifacts" / "phase7_verify" / "verification.json"
