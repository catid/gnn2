...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Recompute and cross-check phase-6 metrics from a saved checkpoint.")
    ...
    parser.add_argument(
        "--eval-config",
        default=None,
        help="Optional evaluation config path. Useful for locked confirmation settings with different benchmark seeds.",
    )
...
def main() -> None:
    ...
    eval_cfg = load_config(args.eval_config) if args.eval_config else base_cfg
    method_name, route_mode, temperature, estimator, total_steps, section_cfg = infer_method_settings(base_cfg)
    train_cfg = eval_cfg["training"]
    system_cfg = eval_cfg.get("system", {})
    ...
    load_checkpoint(checkpoint_path, model, strict=bool(base_cfg.get("resume_strict", True)))

    compute_penalties = current_compute_penalties(eval_cfg["objective"], eval_cfg.get("objective_schedule", {}), total_steps)
    eval_batch_size = int(
        section_cfg.get(
            "eval_batch_size",
    ...
    allow_summary_compare = args.eval_config is None
    ...
    verification = {
        ...
        "eval_config_path": args.eval_config or payload.get("config_path"),
        "method": method_name,
    ...
    out_path = Path(args.out) if args.out else run_dir / "artifacts" / "phase6_verify" / "verification.json"
