...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase-4 audit for final-query control retention.")
    ...
    parser.add_argument("--split", default="test", choices=["val", "test"], help="Primary audit split.")
...
def compute_wait_targets(batch, seq_len: int) -> tuple[torch.Tensor, torch.Tensor]:
    device = batch.labels.device
    dtype = batch.observations.dtype
    targets = torch.zeros(batch.labels.shape[0], seq_len, device=device, dtype=dtype)
    mask = torch.zeros_like(targets)
    if not batch.metadata:
        return targets, mask
    if "trigger_time" not in batch.metadata or "query_time" not in batch.metadata or "needs_final_query" not in batch.metadata:
        return targets, mask
    ...
    for sample_index in torch.nonzero(final_query_mask, as_tuple=False).squeeze(-1).tolist():
        trigger = int(trigger_times[sample_index].item())
        query = int(query_times[sample_index].item())
        trigger = max(0, min(trigger, seq_len - 1))
        query = max(trigger, min(query, seq_len - 1))
        targets[sample_index, trigger : query + 1] = 1.0
        mask[sample_index, trigger : query + 1] = 1.0
    return targets, mask
    ...
    *,
    ...
    with torch.no_grad():
        for step in range(num_batches):
            ...
            wait_targets, wait_mask = compute_wait_targets(batch, batch.observations.shape[1])
            trace["wait_targets"] = wait_targets.detach().cpu()
            trace["wait_mask"] = wait_mask.detach().cpu()
...
def write_probe_plot(probes: dict[str, dict[str, float]], out_path: Path) -> None:
    ...
    plt.ylim(0.0, 1.0)
    plt.title("Needs-Final-Query Probe Accuracy")
...
def build_probe_summary(train_audit: dict[str, Any], test_audit: dict[str, Any]) -> dict[str, dict[str, float]]:
    probe_keys = ["packet_state", "memory_read_state", "router_probs", "control_state"]
    summary: dict[str, dict[str, float]] = {}
    for key in probe_keys:
        train_x, train_y = flatten_probe_tensors(
            train_audit["trace"],
            train_audit["trace"]["wait_targets"],
            train_audit["trace"]["wait_mask"],
            key,
        )
        test_x, test_y = flatten_probe_tensors(
            test_audit["trace"],
            test_audit["trace"]["wait_targets"],
            test_audit["trace"]["wait_mask"],
            key,
        )
...
def main() -> None:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else run_dir / "artifacts" / "phase4_audit"
    ...
    probes = build_probe_summary(probe_train, probe_test)
    ...
    final_query_summary = {
        ...
        "control_probe_packet_state": probes.get("packet_state", {}).get("accuracy"),
        "control_probe_memory_read_state": probes.get("memory_read_state", {}).get("accuracy"),
        "control_probe_control_state": probes.get("control_state", {}).get("accuracy"),
    }
    if final_mask.any():
    ...
    if probes:
        write_probe_plot(probes, out_dir / "probe_accuracy.png")
