...
def find_repo_config_for_run(run_dir: Path) -> Path | None:
    ...
    for basename in basenames:
        ...
        if len(matches) == 1:
            return matches[0]
        if len(matches) > 1:
            preferred = [path for path in matches if "/phase10/dev/" in str(path)]
            if len(preferred) == 1:
                return preferred[0]
...
def main() -> None:
    ...
    out_path = Path(args.out) if args.out else run_dir / "artifacts" / "phase10_verify" / "verification.json"
