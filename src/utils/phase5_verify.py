...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Recompute phase-5 metrics from a saved checkpoint.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Override confirmation batches. Use 0 to skip confirm, -1 to auto-detect.",
    )
...
def resolve_eval_requests(
    *,
    ...
    if args.confirm_batches == 0:
        return requests
    auto_confirm = "confirm" in payload.get("summary", {})
...
def main() -> None:
    ...
    out_path = Path(args.out) if args.out else run_dir / "artifacts" / "phase5_verify" / "verification.json"
